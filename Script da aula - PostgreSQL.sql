...
/* MOSTRANDO A CONFIGURAÇÃO DA DATA*/

SHOW DATESTYLE;

/*
  
  ALTERANDO O TIPO DE DATA PARA D/M/A

  ESSA ALTERAÇÃO DEVE SER REALIZADA NO ARQUIVO DE CONFIGURAÇÃO DENTRO DA PASTA DATA DO POSTGREDQL NO ARQUIVO POSTGRE.CONF.
  PROCURAR A LINHA DATESTYLE.

  C:\Program Files\PostgreSQL\12\data\postgresql.conf

*/

/* INSERIR TODOS OS 1000 DADOS DA TABELA FUNCIONARIOS */

/* FUNÇÕES DE AGREGAÇÃO */

/* QUERY SIMPLES */

/* PARA BI E DATASCIENCE NÃO É INTERESSANTE VER VALORES AVULSO E POR ISSO É UTILIZADO AS AGRAGALÇOES COMO O CONUNT, MAX. MIN, SUM, AVG ENTRE OUTROS PARA ASSIM PODER AGREGAR*/

SELECT * FROM FUNCIONARIOS;

/* CONTANDO O NUMERO DE OCORRENCIAS */

SELECT COUNT(*) FROM FUNCIONARIOS; --CONTA A QUANTIDADE TOTEAL DE FUNCIONARIOS
SELECT COUNT(*) FROM DEPARTAMENTOS; --CONTA A QUANTIDADE TOTAL DE DEPARTAMENTOS
SELECT COUNT(*) FROM LOCALIZACAO; --CONTA A QUANTIDADE DE TOTAL LOCALIZACAO
...
/* MOSTRANDO O NUMERO DE FUNCIONARIOS DE CADA DEPRTAMENTO */

/* NA AULA É UTILIZADO DESSA MANEIRA POIS O PROFESSOR MODELOU AS CHAVES PRIMARIAS COM O PROPRIO NOME DE DEPARTAMENTO NÃO SENDO NECESSARIO A UTILIZAÇÃO DO INNERJOIN*/

SELECT DEPARTAMENTO, COUNT(*) AS "FUNCIONARIOS"
FROM FUNCIONARIOS
GROUP BY DEPARTAMENTO;

/* FOI INSERIDO O MESMO METODO ACIMA, PORÉM, UTLIZANDO O INNER JOIN, QUE FUNCIONA DA MESMA MANEIRA */

SELECT D.DEPARTAMENTO, COUNT(*) AS "FUNCIONARIOS"
FROM FUNCIONARIOS F
...
/* EXIBINDO O MAIXMO E O MINIMO JUNTOS */

SELECT  MIN(SALARIO) AS "SALARIO MAXIMO", 
        MAX(SALARIO) AS "SALARIO MINIMO" 
FROM FUNCIONARIOS;

/* EXIBINDO O MINIMO E O MAXIMO DE CADA DEPARTAMENTO */

/* NA AULA É UTILIZADO DESSA MANEIRA POIS O PROFESSOR MODELOU AS CHAVES PRIMARIAS COM O PROPRIO NOME DE DEPARTAMENTO NÃO SENDO NECESSARIO A UTILIZAÇÃO DO INNERJOIN*/

SELECT  DEPARTAMENTO,
        MIN(SALARIO) AS "SALARIO MAXIMO", 
        MAX(SALARIO) AS "SALARIO MINIMO" 
FROM FUNCIONARIOS
GROUP BY DEPARTAMENTO;
...
SELECT  D.DEPARTAMENTO,
        MIN(F.SALARIO) AS "SALARIO MAXIMO", 
        MAX(F.SALARIO) AS "SALARIO MINIMO" 
FROM FUNCIONARIOS F
    INNER JOIN DEPARTAMENTOS D
    ON D.DEPARTAMENTO = F.DEPARTAMENTO
GROUP BY D.DEPARTAMENTO;
...
SELECT * FROM FUNCIONARIOS
LIMIT 10; --LIMITA A PROJEÇÃO NAS 10 PRIMEIRAS LINHAS

/* QUAL É O GASTO TOTAL DE SALARIO PAGO PELA EMPRESA? */
...
FROM FUNCIONARIOS
GROUP BY DEPARTAMENTO
ORDER BY 3; --PELA TERCEIRA COLUNA
...
FROM FUNCIONARIOS
GROUP BY DEPARTAMENTO
ORDER BY 3 ASC; --PELA TERCEIRA COLUNA ASCENDENTE
...
FROM FUNCIONARIOS
GROUP BY DEPARTAMENTO
ORDER BY 3 DESC; --PELA TERCEIRA COLUNA DESCENDNETE

/*

  CONCORRENDO A VAGA DE CIENTISTADE DADOS JR, VOCÊ RECEBEU PROPOSTAS DE 3 EMPRESAS.
  
  A TABELA A SEGUIR MOSTRA A MÉDIA SALARIOA DAS 3 EMPRESAS EM QUESTÃO:
 ...
 QUAL A SUA ESCOLHA?
 CONSIDERE QUE VOCÊ GOSTOU DAS 3 EMPRESAS IGUALMENTE,
 CONSIDERANDO AMBIENTE DE TRABALHO, CARGA HORÁRIA,
 BENEFÍCIOS, LOCALIZAÇÃO E ETC.

 */

/*
  Em estatística, outlier, valor aberrante ou valor atípico, é uma observação que apresenta um grande afastamento das demais da série[1] (que está "fora" dela), ou que é inconsistente.
...
/*
  BANCO DE DADOS - 1º, 2º E 3º FORMAS NORMAIS
  EVITAM REDUNDANCIA, CONSEQUENTEMENTE POUPAM ESPAÇO EM DISCO.
  CONSOMEM MUITO PROCESSAMENTO EM FUNÇÃO DE JOINS. QUERYS LENTAS.

  DATA SCIENCE E B.I - FOCAM EM AGREGAÇÕES E PERFOMANCE. NÃO SE
  PREOCUPAM COM ESPAÇO EM DISCO. EM B.I, MODELAGEM MINIMA (DATA WAREHOUSE)
...
/* LOCAL DO ARQUIVO CSV C:\Scripts SQL DataScience\LogMaquinas.csv */

COPY MAQUINAS --COPIA PARA A TABELA
FROM 'C:\Scripts SQL DataScience\LogMaquinas.csv' --O ARQUIVO DE LogMaquinas.csv
DELIMITER ',' --DELIMITADOR DO ARQUIVO CSV QUE SEPARA AS COLUNAS
CSV HEADER; --DIZ QUE A PRIMEIRA LINHA É UM CABEÇALHO
...
ROUND(COLUNA,2) -- MESMA COISA QUE TRUNCATE(COLUNA, CASAS DECIMAIS) NO MYSQL

SELECT MAQUINA,
        ROUND(AVG(QTD),2) AS "QUANTIDADE MÉDIA"
FROM MAQUINAS
GROUP BY MAQUINA
ORDER BY 2 DESC;

/* QUAL É A MODA DA QUANTIDADE DE CAA MAQUINA */

SELECT  MAQUINA AS "MAQUINA",
    QTD AS "QUANTIDADE",
    COUNT(*) AS "MODA" --PODEMOS USAR O COUNT COMO MODA
FROM MAQUINAS
GROUP BY MAQUINA, QTD
ORDER BY 3 DESC;

/* QUAL A MODA DAS QUANTIDADES DE CAD MAQUINA */

SELECT MAQUINA, QTD, COUNT(*) AS "MODA"
FROM MAQUINAS
WHERE MAQUINA = 'Maquina 01' --TEM QUE OBEDECER O CASE SENSITIVE
GROUP BY MAQUINA, QTD
ORDER BY 3 DESC
LIMIT 1;
...
/*
  CONTAGEM DE OCORRÊNCIA = HISTOGRAMA
  HISTOGRAMA É UM GRAFICO ONDE É INSERIDO A QUANTIDADE DE VEZES QUE ALGO OCORREU.
*/


/* QUAL O MAXIMO E O MINMO E A AMPLITUDE DE CADA MAQUINA? */
...
STDDEV_POP(COLUNA) --DESVIO PADRÃO DA POPULAÇÃO DA COLUNA

VAR_POP(COLUNA) --VARIANCIA DA POPULAÇÃO DA COLUNA
...
LEAGUAGE 'sql' IMMUTABLE;

CREATE AGREGATE MEDIAN (NUMERIC) (
  SFUNC=ARRAY_APPEND,
  STYPE=NUMERIC[],
...
(STDDEV_POP(QTD) / AVG(QTD) * 100) AS "COEF. VAR." -- DESVIO PADRÃO / MEDIA * 100 = COEFICIENTE DE VARIANÇÃO
...
/*
  AO UTILIZAR ESSA FUNÇÃO SEM O ORDER BY O RESULTADO SERA A MODA INTEIRA DA COLUNA MAQUINA, SENDO
  QUE O CORRETO É TRAZER A MODA DAS MAQUINAS 1, 2 E 3 EM REGISTROS SEPARADOS.
...
/* 
  ...
  PARA NÃO OCORRER ERROS DE NOMES DE COLUNAS DUPLICADOS PODEMOS COLOCAR UM ALIAS NAS COLUNAS
  PARA QUE QUANDO SEJA EXPORTADO EM CSV POSSAMOS IDENTIFICAR AS COLUNAS CORRETAMENTE
...
COPY REL_LOCADORA
TO 'C:\Scripts SQL DataScience\REL_LOCADORA.csv'
DELIMITER';'
CSV HEADER;
...
/*
    Os objetos de seqüência (também chamados de geradores de seqüência,
    ou simplesmente de seqüências), são tabelas especiais de um única linha criadas pelo comando CREATE SEQUENCE. O objeto de seqüência é usado normalmente
    para gerar identificadores únicos para linhas de tabelas. As funções de seqüência, listadas na Tabela abaixo, fornecem métodos simples, 
    seguros para multiusuários, para obter valores sucessivos da seqüência a partir dos objetos de seqüência.
*/
+-------------------------------+----------------+----------------------------------------------------------------+
|              Função           | Tipo retornado |                         Descrição                              |
+-------------------------------+----------------+----------------------------------------------------------------+
| NEXTVAL(text)                 | bigint         | Avança a seqüência e retorna o novo valores                    |
...
NEXTVAL('SEQ_LOCACAO'); -- UTILIZAR NO LUGAR DO CAMPO ID DA CHAVE PRIMARIA (EQUIVALE AO AUTO_INCREMENT DO MYSQL)
...
SELECT  L.IDLOCACAO,
        F.NOME AS "FILME",
        G.NOME AS "GENERO",
        L.DATA AS "DATA", L.DIAS AS "DIAS", L.MIDIA AS "MIDIA"
FROM GENERO G 
INNER JOIN FILME F
ON F.ID_GENERO = G.IDGENERO
INNER JOIN LOCACAO L
...
/* SELECT PARA TRAZER OS REGISTROS NOVOS USANDO SUB-QUERYS*/

SELECT MAX(IDLOCACAO) AS RELATORIO,
(SELECT MAX(IDLOCACAO) FROM LOCACAO) AS LOCACAO
FROM RELATORIO_LOCADORA;

/* SE OS NUMEROS DOS IDS FOREM IGUAIS, SIGNIFICA QUE AS TABLEAS ESTÃO SINCRONIZADAS */
...
/* SE O RESULTADO FOR UM EMPITY SET, SIGNIFICA QUE AS TABELAS ESTÃO SINCRONIZADAS. */
...
/* RETORNARÁ 0 DEVIDO AS TABLEAS ESTAREM SINCRONIZADAS */

/* AULA 132. 19 Programando a Sincronização */

/* DEIXANDO ESSE PROCEDIMENTO AUTOMÁTICO POR MEIO DE UMA TRIGGER */

/* 

  A TRIGGER NO POSTGRESQL É UM POUCO DIFERENDE DE OUTROS BANCOS.
  NO MYSQL, POR EXEMPLO, CRIAMOS A PROGRAMAÇÃO DENTRO DA TRIGGER E ESSA POR SUA VEZ AGUARDA
  ALGO OCORRER PARA ACIONAR O GATILHO DA PROGRAMAÇÃO DELA, AO PASSO QUE
  NO POSTGRESQL A PROGRAMAÇÃO É FEITA SEPARADAMENTE EM UMA FUNCTION
  PARA DEPOIS CRIAR UMA TRIGGER QUE AO SER ACIONADA IRA CHAMAR A FUNCTION E ESSA POR SUA VEZ
  REALIZARA A TAREFA PROGRAMADA. 
...
BEGIN
    ...
    COPY RELATORIO_LOCADORA TO 'C:\Scripts SQL DataScience\REL_LOCADORA.csv'
    DELIMITER ';'
    CSV HEADER;
...
INSERT INTO LOCACAO VALUES(NEXTVAL('SEQ_LOCACAO'),NOW(),200,1,200);
INSERT INTO LOCACAO VALUES(NEXTVAL('SEQ_LOCACAO'),NOW(),800,6,500);

SELECT * FROM LOCACAO;
SELECT * FROM RELATORIO_LOCADORA;

/* SINCRONIXANDO REGISTROS DELETADOS */
...
/* QUAL A MODA DOS SALARIOS. A MODA DOS SALARIOS DIZ ALGO RELEVANTE? */
...
--A MODA DIZ QUE O DEPARTAMENTO QUE TEM O MAIOR SALARIO É O DE JARDIM
...
SELECT  DEPARTAMENTO,
        COUNT(DEPARTAMENTO) AS "QUANTIDADE",
        MODE() WITHIN GROUP(ORDER BY SALARIO) AS "MODA"       
FROM FUNCIONARIOS
GROUP BY DEPARTAMENTO
ORDER BY 2 DESC;

--OS DEPARTAMENTOS QUE MAIS EMPREGAM SÃO OS DE ROUPAS E BELEZA.
...
SELECT  DEPARTAMENTO,
        ROUND(STDDEV_POP(SALARIO),2) AS "DESVIO PADRÃO"
FROM FUNCIONARIOS
GROUP BY DEPARTAMENTO
ORDER BY 2 DESC;
...
/* CALCULE A MEDIANA SALARIAL DE TODO SET DE DADOS. */
...
/* CALCULE AS PRINCIPAIOS MEDIDAS ESTATISTICAS POR DEPARTAMENTO */
...
SELECT  DEPARTAMENTO,
        ...
        MODE() WITHIN GROUP(ORDER BY SALARIO) AS "MODA",
        MAX(SALARIO) - MIN(SALARIO) AS "AMPLITUDE",
        ROUND(VAR_POP(SALARIO),2) AS "VARIÂNCIA",
        ROUND(STDDEV_POP(SALARIO),2) AS "DESVIO PADRÃO",
...
/* QUAL DEPARTAMENTO DÁ UMA MAIOR PROBABILIDADE DE GANHAR MAIS? (OUTDOOR) */
...
SELECT  NOME, CARGO,
        CASE
            ...
            WHEN CARGO = 'Structural Engineer' THEN 'CONDICAO 2'
            WHEN CARGO = 'Executive Secretary' THEN 'CONDICAO 3'
            WHEN CARGO = 'Sales Assistant' THEN 'CONDICAO 4'
...
/* MESCLANDO TECNICAS - VARIABLE DUMMY*/
...
SELECT NOME, DEPARTAMENTO, SALARIO
FROM FUNCIONARIOS
WHERE DEPARTAMENTO = 'Ferramentas'; --RESPEITAR O CASE SENSITIVE
...
/*
  ...
  FILTROS BASEADO EM UNICO TIPO E COLUNA - ATENÇÃO A REGRA DO AND E OR
...
/* FILTROS BASEADOS EM PADRÃO CARACTERES */
...
/* FILTROS BASEADOS EM PADRÃO CARACTERES COM MAIS LETRAS */
...
/* UTILIZANDO O CARACTERE CORINGA NO MEIO DA PALAVRA */
...
/* 
    E SE EU QUISER FILTRAR O AGRUPAMENTO PELO SALARIO?
    POR EXEMPLO MAIOR QUE 40000000
...
--QUERY TRARÁ ERRO DEVIDO A GAVER MESCLA DE COLUNAS COMUNS E COLUNAS AGREGADAS
...
--PARA RESOLVER A QUESTÃO DA AGREGAÇÃO SUBSTITUIR O WHERE PELO HAVING (É O WHERE DAS CLÁUSULAS AGREGADAS)
...
-- DEVIO ESSA QUERY SER MTO GRANDE E POSSUIR SUB-QUERY, PODE OCORRER DEGRADAÇÃO DE PERFORMANCE NO COMPUTADOR. (UTILIZAR COM CUIDADO)

SELECT  COUNT (*) AS "QUANTIDADE TOTAL",
        COUNT (*) FILTER (WHERE SEXO = 'Masculino') AS "MASCULINO"
FROM FUNCIONARIOS;

-- DA MANEIRA ACIMA É POSSÍVEL UTILIZAR A CLAUSULA FILTER PARA FILTRAR A COLUNA QUE SE QUER PROJETAR
...
/* DISTINNCT - TRAZ CAMPOS SEM SE REPETIREM */

SELECT DISTINCT DEPARTAMENTO
FROM FUNCIONARIOS;

/*
  FUNÇÃO UPPER CASE - DEIXA TUDO EM CAIXA ALTA
  FUNÇÃO LOWER CASE - DEIXA TUDO MINUSCULO
  FUNÇÃO CAMEL CASE - PRIMEIRA LETRA MAIUSCULA RESTANTE MINUSCULO.
*/
...
/* CONTANDO CARACTERES - FUNÇÃO LENGHT */

SELECT LENGHT ('   UNIDADOS    ');

/*APLICANDO O TRIM */

SELECT TRIM ('   UNIDADOS    ');

SELECT LENGHT (TRIM ('   UNIDADOS    '));

/* DESAFIO - CRIAR UMA COLUNA AOLADO DA COLUNA CARGO QUE DIGA SE A PESSOA É ASSISTENTE OU NÃO */
